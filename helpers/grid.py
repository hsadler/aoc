...
class Point:
    ...
    def __repr__(self):
        return (
            f"Point({self.x}, {self.y}, {self.val}), color={self.get_color_as_tuple()}"
        )
...
class Grid:
    ...
    def get_width(self) -> int:
        return len([p.x for p in self.get_points() if p.y == 0])

    def get_height(self) -> int:
        return len([p.y for p in self.get_points() if p.x == 0])
    ...
    def to_matrix(self) -> list[list[str]]:
        ...
        for y in range(self.get_height()):
            row = self.get_row(y)
            matrix.append([p.val for p in row])
        return matrix
        ...
        while curr_y in y_to_row:
            ...
            for p in row:
                print_strings.append(
                    functions.get_color_string(color_map[p.color], p.val)
                )
            print_strings.append("\n")
            curr_y += 1
...
def get_rotated_grid(g: Grid) -> Grid:
    rotated_matrix = functions.rotate_matrix(g.to_matrix())
    return Grid.build(rotated_matrix)


def get_transposed_grid(g: Grid) -> Grid:
    transposed_matrix = functions.transpose_matrix(g.to_matrix())
    return Grid.build(transposed_matrix)
