...
class Grid:
    ...
    def get_first_int_points(self) -> list[Point]:
        def point_is_first_int(p: Point) -> bool:
            ...
            return p.val.isdigit() and (
                left_neighbor is None or not left_neighbor.val.isdigit()
            )
...
if True:
    ...
    for p in gears:
        ...
        number_neighbors = Grid.filter_points(
            grid.get_neighbors(p), lambda n: n.val.isdigit()
        )
        ...
        for n in number_neighbors:
            ...
            coord_to_number_points[
                functions.serialize_position(first_number_point.x, first_number_point.y)
            ] = number_points
        nums_to_product = []
